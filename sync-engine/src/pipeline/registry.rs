...
type TransformFactory =
    Box<dyn Fn() -> Pin<Box<dyn Future<Output = Result<Box<dyn AnyTransform>>> + Send>> + Send + Sync>;
type SinkFactory =
...
impl Registry {
    ...
    {
        self.sources.insert(key.to_owned(), Box::new(move || Box::pin(factory())));
    }
    ...
    {
        self.transforms.insert(key.to_owned(), Box::new(move || Box::pin(factory())));
    }
    ...
    {
        self.sinks.insert(key.to_owned(), Box::new(move || Box::pin(factory())));
    }
    ...
    pub async fn run_from_toml(self, path: &str) -> Result<()> {
        ...
        for entry in def.pipeline {
            let label = entry.name.clone().unwrap_or_else(|| {
                format!("{}/{}/{}", entry.source, entry.transform, entry.sink)
            });
            ...
            scheduler
                .add(Job::new_async(entry.cron.as_str(), move |_, _| {
                    let (reg, sk, tk, kk, lbl) =
                        (Arc::clone(&reg2), sk.clone(), tk.clone(), kk.clone(), lbl.clone());
                    Box::pin(async move {
                        info!(pipeline = %lbl, "Cycle starting");
                        let mut source = match reg.sources[&sk]().await {
                            Ok(s)  => s,
                            Err(e) => { tracing::error!(error = %e, "Source build failed"); return; }
                        };
                        let transform = match reg.transforms[&tk]().await {
                            Ok(t)  => t,
                            Err(e) => { tracing::error!(error = %e, "Transform build failed"); return; }
                        };
                        let sink = match reg.sinks[&kk]().await {
                            Ok(s)  => s,
                            Err(e) => { tracing::error!(error = %e, "Sink build failed"); return; }
                        };
